...
async def test_genre_list_200(session_client, es_client, redis_client):
    ...
    response_api = await session_client.get(f"{test_settings.service_url}{test_settings.genres_endpoint}")
    ...
    response_redis = await redis_get(redis_client, f"{test_settings.service_url}{test_settings.genres_endpoint}")
...
@pytest.mark.parametrize(
    "response, code_result", [
        (f"{test_settings.service_url}{test_settings.genres_endpoint}?page[number]=0&page[size]=5", 422),
        (f"{test_settings.service_url}{test_settings.genres_endpoint}?page[number]=-1&page[size]=5", 422)
    ]
)
@pytest.mark.asyncio
...
async def test_genre_by_id_200(session_client, es_client, redis_client, genre_id, status_code):
    ...
    response_api = await session_client.get(f"{test_settings.service_url}{test_settings.genres_endpoint}{genre_id}")
    assert response_api.status == status_code
    response_api = await response_api.json()

    response_elastic = await elastic_search_by_id(es_client, test_settings.genres_index, genre_id)

    response_redis = await redis_get(redis_client, f"{test_settings.service_url}"
                                                   f"{test_settings.genres_endpoint}"
                                                   f"{genre_id}")
...
async def test_genre_by_id_404(session_client):
    ...
    response_api = await session_client.get(f"{test_settings.service_url}{test_settings.genres_endpoint}/bad_genre_id")
    assert response_api.status == 404

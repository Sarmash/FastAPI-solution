...
@pytest.mark.asyncio
async def test_films_search_200(session_client, es_client, redis_client, query, status_code):
    """
    ...
    request_url = (f"{test_settings.service_url}"
                   f"{test_settings.search_films_endpoint}"
                   f"?query={query}")

    response_api = await http_request(session_client, request_url, status_code)

    response_elastic = await elastic_search_list(es_client, index=test_settings.movies_index, body={
...
@pytest.mark.asyncio
async def test_pagination_films_search_200(session_client, query, size, status_code, movies):
...
@pytest.mark.asyncio
async def test_person_search_200(session_client, es_client, redis_client, query, status_code):
    ...
    response_elastic = await elastic_search_list(es_client,
...
@pytest.mark.parametrize(
    "query, status_code, films, size", [
        ('Fred+Olen+Ray', 200, 1, 1),
        ('Suzy+Stokey', 200, 2, 2)
    ]
)
@pytest.mark.asyncio
async def test_pagination_persons_search_200(session_client, query, size, status_code, films):
    """Проверка вывода N-го числа данных с помощью пагинации"""
